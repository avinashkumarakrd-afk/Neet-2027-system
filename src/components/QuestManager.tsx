...
import { Plus, Trash2, CheckCircle, Circle, Target, Beaker, Dna, FileText } from 'lucide-react';
...
export default function QuestManager({ quests, onAddQuest, onToggleQuest, onDeleteQuest }: QuestManagerProps) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#121212] border border-[#222] overflow-hidden">
      <div className="p-5 border-b border-[#222] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <h2 className="text-xs uppercase tracking-widest font-bold border-l-2 border-[#00F0FF] pl-2 flex gap-4">
          <button 
      ...
      {/* Add Task Form */}
      {activeTab === 'active' && (
      <form onSubmit={handleAdd} className="p-4 border-b border-[#222] bg-[#1A1A1A]/30 flex flex-col sm:flex-row gap-3">
        <input 
          type="text" 
          value={newTaskTitle}
          onChange={e => setNewTaskTitle(e.target.value)}
          placeholder="New Objective..."
          className="flex-1 bg-transparent border-b border-[#333] px-2 py-2 text-sm text-[#E0E0E0] placeholder-[#555] focus:outline-none focus:border-[#00F0FF] transition-colors"
        />
        <div className="flex gap-3 items-center">
          <select 
            value={newTaskStat}
            onChange={e => setNewTaskStat(e.target.value as StatType)}
            className="bg-[#1A1A1A] border border-[#333] rounded-none px-3 py-1.5 text-[11px] font-mono text-[#E0E0E0] focus:outline-none focus:border-[#00F0FF] uppercase"
          >
          ...
          <input 
            ...
            min="0.1"
            max="100"
            className="w-16 bg-[#1A1A1A] border border-[#333] rounded-none px-3 py-1.5 text-[11px] font-mono text-[#E0E0E0] focus:outline-none focus:border-[#00F0FF]"
            placeholder="CH"
          />
          <button 
            type="submit"
            disabled={!newTaskTitle.trim()}
            className="bg-[#00F0FF] text-[#0A0A0B] px-4 py-1.5 text-[10px] font-black uppercase tracking-tighter disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[#00F0FF]/80 transition-colors"
          >
            + Add Goal
          </button>
        </div>
          ...
          {activeTab === 'active' && activeQuests.map((quest) => {
            ...
            return (
              <motion.div
                ...
                className="group bg-[#1A1A1A] border border-[#333] hover:border-[#00F0FF]/50 p-4 flex items-center gap-4 transition-colors"
              >
                <button 
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                  ...
                  <div className="text-[10px] text-[#00F0FF]/60 font-mono mt-0.5 flex items-center gap-1 uppercase">
                ...
                <button 
                  ...
                  className="text-[10px] text-[#555] group-hover:text-red-500 px-3 py-1 border border-[#333] group-hover:border-red-500/30 shrink-0 transition-colors uppercase tracking-widest"
                >
                  Delete
                </button>
              </motion.div>
          ...
          {activeTab === 'history' && completedQuests.map((quest) => {
            return (
              ...
              >
                <button 
                  onClick={() => onToggleQuest(quest.id)}
                  className="w-4 h-4 border border-[#00F0FF] flex items-center justify-center bg-[#00F0FF]/20 shrink-0"
                >
                ...
                <div className="flex-1 overflow-hidden flex flex-col">
                  ...
                  <div className="text-[10px] text-[#00F0FF]/40 font-mono mt-0.5 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 uppercase">
                    <span>REWARD: +{quest.chaptersValue} {quest.stat} CH</span>
                    {quest.completedAt && (
